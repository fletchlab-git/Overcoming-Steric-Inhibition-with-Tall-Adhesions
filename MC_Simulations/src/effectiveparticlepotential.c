...
double ReturnConvParticlePotentialLatticeSite(sys *Sys, double dz, double sqrtnormbot, double sqrtnormtop){
  ...
  return -(log(expveff) + Sys->VeffZero)*Sys->VeffCGFact;
}

double ReturnParticleLambPotentialLatticeSite(sys *Sys, double dz, double sqrtnormbot, double sqrtnormtop){
  return ReturnConvParticlePotentialLatticeSite(Sys, dz, sqrtnormbot, sqrtnormtop)*Sys->MeshBias.BiasStr;
}
...
void SetVeffLatticeSite(sys *Sys, int i0, int j0){
  ...
  double sqrtnormbot = 1.0; //Sys->BotMesh[i0][j0].nNorm;
  double sqrtnormtop = 1.0; //Sys->TopMesh[i0][j0].nNorm;
...
void ComputeParticlePops(sys *Sys){
  int i, j, p1, p2, Nx = Sys->Nx, Ny = Sys->Ny, compnums = Sys->NumPTypes + 1;
