...
int CheckMesh_Consistency(sys *Sys){
  int Err=0, i, j;
  double top = 0.0, bot = 0.0;
  
  for(i=0;i<Sys->Nx;i++){
    for(j=0;j<Sys->Ny;j++){
      if(fabs(Sys->TopMesh[i][j].Pos.z - Sys->BotMesh[i][j].Pos.z) > 1e6){printf("\n\nCrossing\n\n"); Err = 2;}
      top += Sys->TopMesh[i][j].Pos.z;
      bot += Sys->BotMesh[i][j].Pos.z; 
    }
  }
  top /= Sys->MTot;
  bot /= Sys->MTot;
//  if(fabs(Sys->MeshBias.thetaBot - bot) > 1e-6) Err = 2;
  return Err; 
}
...
int GlobalMeshShift(sys *Sys){
  Sys->suggestedz = gsl_ran_flat(Sys->rng, -Sys->GlobShiftSize, Sys->GlobShiftSize);
...
void ChooseNodePBC(int *i, int *j, int Nx, int Ny, gsl_rng *rng){
  *i=gsl_rng_uniform_int(rng, Nx);
  *j=gsl_rng_uniform_int(rng, Ny);
}

void ChooseNodeFrame(int *i, int *j, int Nx, int Ny, gsl_rng *rng){ 
  *i=gsl_rng_uniform_int(rng, Nx - 2) + 1;
  *j=gsl_rng_uniform_int(rng, Ny - 2) + 1;
}
  
void SuggestMeshMoveEQ(sys *Sys){
  Sys->i0 = Sys->j0 = -1;
  int Nmin = (int)round(0.5*Sys->Nx) - 1;
  int Nmax = (int)round(0.5*Sys->Nx) + 1;
  
  while( Sys->i0 < 0 || 
         (Sys->i0 >= Nmin && Sys->i0 <= Nmax && 
          Sys->j0 >= Nmin && Sys->j0 <= Nmax   )){
    ChooseNode(&(Sys->i0), &(Sys->j0), Sys->Nx, Sys->Ny, Sys->rng);
  }
...
void SuggestMeshMoveLambda(sys *Sys){
